...
import           Construction (Name, Term (..), bound, equal, free, fresh, alpha, beta, eta, reduce, substitute)
import           Test.Hspec
import Data.Set
...
testEqual = do
    ...
    it "#2" $ equal varX1 varX `shouldBe` False
    it "#3" $ equal (Lam "x" varX) (Lam "y" (Var "y")) `shouldBe` True
    it "#4" $ equal (Lam "x" (App (Var "x") (Var "y"))) (Lam "a" (App (Var "a") (Var "y"))) `shouldBe` True
    it "#5" $ equal (App lamK lamId) (App lamId (App lamId lamKK)) `shouldBe` True
